...
def get_user_inputs():
    ...
    speed_mode = input(f"Enter speed mode (fast, medium, slow, random) (default {DEFAULT_SPEED_MODE}): ").strip().lower() or DEFAULT_SPEED_MODE
...
def set_delay(speed_mode):
    ...
    delays = {
        ...
        "medium": 1,
        "slow": 5,
        "random": random.uniform(0.1, 10)
    }
    return delays.get(speed_mode, random.uniform(0.1, 10))
...
def click_follow_button(button, delay, username, follow_count):
    """Clicks a follow button with a delay and logs user info."""
    try:
        button.click()
        follow_count += 1
